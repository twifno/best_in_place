...
BestInPlaceEditor.prototype = {
  ...
  abortIfConfirm : function () {
    if (!this.useConfirm) {
      this.abort();
      if(this.element != this.activator){
        jQuery(this.activator).show();
      }
      return;
    }
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.content_id        = self.content_id        || $parent.attr("data-content-id");
      self.content_bar_id    = self.content_bar_id    || $parent.attr("data-content-bar-id");
      self.content_class     = self.content_id        || $parent.attr("data-content-class");
      self.content_bar_class = self.content_bar_id    || $parent.attr("data-content-bar-class");
      self.content_real_id   = self.content_real_id   || $parent.attr("data-content-real-id");
      self.options_wrapper_class = self.options_wrapper_class  || $parent.attr("data-options-wrapper-class");
      self.options_list_class    = self.options_list_class     || $parent.attr("data-options-list-class");
    });
    ...
    self.content_id        = self.element.attr("data-content-id")          || self.content_id;
    self.content_bar_id    = self.element.attr("data-content-bar-id")      || self.content_bar_id;
    self.content_class     = self.element.attr("data-content-class")       || self.content_class;
    self.content_bar_class = self.element.attr("data-content-bar-class")   || self.content_bar_class;
    self.content_real_id   = self.element.attr("data-content-real-id")     || self.content_real_id;
    self.options_wrapper_class = self.element.attr("data-options-wrapper-class")  || self.options_wrapper_class;
    self.options_list_class    = self.element.attr("data-options-list-class")     || self.options_list_class;
  ...
  clickHandler : function(event) {
    if(this == event.data.editor.activator){
      jQuery(this).hide();
    }
    ...
    inputBlurHandler : function(event) {
      if (event.data.editor.okButton) {
        ...
        if (event.data.editor.cancelButton) {
          ...
          event.data.editor.update();
          if(event.data.editor.element != event.data.editor.activator){
            jQuery(event.data.editor.activator).show();
          }
        }
      }
    },
  ...
  "textarea" : {
    activateForm : function() {
      ...
      var output   = jQuery(document.createElement('form'))
                     ...
                     .attr('id', this.content_bar_id)
                     .attr('class', this.content_bar_class))
                     .append(jQuery(document.createElement('textarea'))
                     .val(this.sanitizeValue(this.display_value))
                     .attr('id', this.content_id)
                     .attr('class', this.content_class))
                     .append(jQuery(document.createElement('div'))
                     .attr('class', this.options_wrapper_class)
                     .append(jQuery(document.createElement('div'))
                       .attr('class', this.options_list_class))
                            );
      this.element.html(output);
      ...
      jQuery(this.element.find("."+this.options_list_class)[0]).html(output.html());
      ...
      var editor = new Markdown.Editor(converter, this.content_real_id);
      editor.run();
    ...
    cancelButtonHandler : function(event) {
      ...
      clearTimeout(event.data.editor.blurTimer);
      //event.data.editor.abortIfConfirm();
      event.data.editor.abort(); //no confirmation for cancel
      event.stopPropagation(); // Without this, click isn't handled
      if(event.data.editor.element != event.data.editor.activator){
        jQuery(event.data.editor.activator).show();
      }
